...
from middleware.redact import redact_pii_middleware
...
agent = define_deep_agent(
    ...
    model=FAST_MODEL,
    # Middleware order matters and MDA never infers it. Redaction is declared
    # first so it runs before the router hands the task to TypeSafe. Auto mode
    # gates tool calls rather than the model, so it sits outside that ordering.
    middleware=[
